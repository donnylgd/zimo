...
import { ArrowLeft, Download, Copy, CheckCircle2, FileSpreadsheet, AlertCircle, ChevronLeft, ChevronRight, Info, Loader2 } from 'lucide-react';
          ...
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {status && (
              <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-tight ${
                ...
                {status === 'completed' ? t.history.status.completed : status === 'processing' ? t.history.status.processing : t.history.status.failed}
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-500/5 rounded-lg border border-emerald-100 dark:border-emerald-500/20">
                ...
                <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-tight">{t.task_detail.success_count.replace('{count}', successCount.toString())}</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 dark:bg-amber-500/5 rounded-lg border border-amber-100 dark:border-amber-500/20">
                ...
                <span className="text-[10px] font-bold text-amber-700 dark:text-amber-400 uppercase tracking-tight">{t.task_detail.failure_count.replace('{count}', failureCount.toString())}</span>
              </div>
              
              {/* Failure Hint - Default */}
              {filter === 'all' && failureCount > 0 && (
                      ...
                      <td className="px-6 py-5">
                        {row.status === 'success' ? (
                          ...
                          <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 italic">
                            ...
                            <span>{t.task_detail.failure_reason_default}</span>
                          </div>
                        )}
                      </td>
